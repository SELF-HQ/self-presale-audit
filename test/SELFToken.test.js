...
describe("SELFToken", function () {
  ...
  describe("Deployment", function () {
    ...
    it("Should have 18 decimals", async function () {
      expect(await selfToken.decimals()).to.equal(18);
    });

    it("Should mint total supply to deployer", async function () {
      const totalSupply = ethers.utils.parseEther("500000000"); // 500M
  ...
  describe("Transfers", function () {
    ...
    it("Should fail if sender doesn't have enough tokens", async function () {
      ...
      await expect(
        selfToken.connect(user1).transfer(user2.address, ethers.utils.parseEther("1"))
  ...
  describe("Ownership", function () {
    ...
    it("Should reject renouncing ownership to zero address", async function () {
      await expect(
        selfToken.renounceOwnership()
      ).to.not.be.reverted;
      
      expect(await selfToken.owner()).to.equal(ethers.constants.AddressZero);
    });
  });
